...
namespace WindowsFormsApp4
{
    ...
    {
        private static ImagingFactory Imgfactory; // = new ImagingFactory();
        ...
        public static Texture2D CreateTex2DCubeFromFile(Device device, string[] filename)
        {
            BitmapSource[] bSource = new BitmapSource[6];

            for (int i = 0; i < 6; i++)
            {
                bSource[i] = LoadBitmap(filename[i]);
            }

            return CreateTex2DCubeFromBitmap(device, bSource);
        }

        private static Texture2D CreateTex2DCubeFromBitmap(Device device, BitmapSource[] bsource)
        {

            Texture2DDescription desc;
            desc.Width = bsource[0].Size.Width;
            desc.Height = bsource[0].Size.Height;
            desc.ArraySize = 6;
            desc.BindFlags = BindFlags.ShaderResource;
            desc.Usage = ResourceUsage.Default;
            desc.CpuAccessFlags = CpuAccessFlags.None;
            ...
            DataRectangle[] rect = new DataRectangle[6];

            for (int i = 0; i < 6; i++)
            {
                var s = new DataStream(bsource[i].Size.Height * bsource[i].Size.Width * 4, true, true);
                bsource[i].CopyPixels(bsource[i].Size.Width * 4, s);

                rect[i] = new DataRectangle(s.DataPointer, bsource[i].Size.Width * 4);
            }
